...
* <U>Project Objective</U>: Devise backscatter communication techniques for ambient IoT 
	- High-speed passive backscatter communication over ranges of meters 
		...
		The classical UHD RFID system only supports communication rate of several to a few hundred kbps, which limits the backscatter communication in the simple data collection applications like inventory and tracking. In fact, the backscatter communication technology can be envisioned to be applied in more energy-constrained scenarios with higher data-rate requirements. We developed a SDR based hardware platform with baseband signal processing techniques including detection, synchronization, equalization, interference cancellation, demodulation etc. Accordingly, we are continuously improving the hardware of our own backscatter devices (advanced backscatter tags) to achieve higher communication rates. Experimental results show that the hardware platform can communicate with the our tag at transmission rates as high as 5~10 Mbps over a range of a few meters.
	<figure>
